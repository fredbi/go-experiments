...
func init() {
	gzWriterPool = new(sync.Pool)
	gzWriterPool.New = func() interface{} {
...
func MarshalGzipped(layers Layers) ([]byte, error) {
	...
	gzw := gzWriterPool.Get().(*gzip.Writer)
	gzw.Reset(buf)
	defer func() {
		if err := gzw.Close(); err != nil {
			fmt.Fprintf(os.Stderr, "closing gzip writer: %v\n", err)
		}
		gzWriterPool.Put(gzw)
	}()

	if _, err := gzw.Write(data); err != nil {
		return nil, fmt.Errorf("writing gzip of tile: %v", err)
	}
	if err := gzw.Flush(); err != nil {
		return nil, fmt.Errorf("flushing gzip of tile: %v", err)
	}
	if err := gzw.Close(); err != nil {
		return nil, fmt.Errorf("closing gzip of tile: %v", err)
	}
